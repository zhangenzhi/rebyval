...
class Cifar10DataLoader(BaseDataLoader):
    ...
    def load_dataset_to_device(self, epoch=1, device_name="/GPU:0"):
        ...
        np.random.shuffle(train_bundle)
        x_train, y_train = zip(*train_bundle)

        x_test = x_test.astype(np.float32)
        y_test = y_test.astype(np.float32)
        test_bundle = list(zip(x_test, y_test))
        np.random.shuffle(test_bundle)
        x_test, y_test = zip(*test_bundle)
        ...
        train_set = DatasetWrapper({"inputs": train_data, "labels": train_label})
